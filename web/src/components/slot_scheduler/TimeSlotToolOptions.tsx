...
import { useTimeSlotFormContext } from '@/components/slot_scheduler/TimeSlotFormProvider.tsx';
...
export function TimeSlotToolOptions({ onClose }: Props) {
  ...
  const { getMaterializedProgramList } = useChannelEditorLazy();
  ...
  const handleAutoSlots = () => {
    let startTime = 0;
    const programs = getMaterializedProgramList();
    ...
    const slots = seq.collect(programOptions, (option) => {
      if (option.type === 'show' && !isEmpty(programsByShowId[option.showId])) {
        ...
        const duration =
          Math.round(dayjs.duration(maxDuration).asMinutes() / 5) * 5;
        const slot = {
          startTime,
          order: 'next',
          programming: {
