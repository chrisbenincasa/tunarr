...
export type InfiniteSlotType = (typeof InfiniteSlotTypes)[number];

export const AnchorModes = ['hard', 'soft', 'padded'] as const;
export type AnchorMode = (typeof AnchorModes)[number];
...
export type IterationDirection = (typeof IterationDirections)[number];

/**
 * Configuration for a slot that is stored as JSON.
 * Contains iteration settings, filters, etc.
 */
export type InfiniteSlotConfig = {
...
export const InfiniteScheduleSlot = sqliteTable(
  'infinite_schedule_slot',
  {
    ...
    showId: text().references(() => ProgramGrouping.uuid, { onDelete: 'cascade' }),
    customShowId: text().references(() => CustomShow.uuid, { onDelete: 'cascade' }),
    fillerListId: text().references(() => FillerShow.uuid, { onDelete: 'cascade' }),
    redirectChannelId: text().references(() => Channel.uuid, { onDelete: 'cascade' }),
    smartCollectionId: text().references(() => SmartCollection.uuid, { onDelete: 'cascade' }),
  ...
  (table) => [
    ...
    index('infinite_schedule_slot_redirect_channel_id_index').on(table.redirectChannelId),
    index('infinite_schedule_slot_smart_collection_id_index').on(table.smartCollectionId),
  ],
);
...
export type InfiniteScheduleSlot = InferSelectModel<typeof InfiniteScheduleSlot>;
export type NewInfiniteScheduleSlot = InferInsertModel<typeof InfiniteScheduleSlot>;
