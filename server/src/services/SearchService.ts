...
import { isDefined, isNonEmptyString, isWindows, wait } from '../util/index.ts';
...
export type ProgramSearchDocument<
  Type extends ProgramType | ProgramGroupingType =
    | ProgramType
    | ProgramGroupingType,
> = {
  id: string;
...
export class MeilisearchService implements SearchService {
          ...
          if (!isMainThread) {
            pm2.list((err, processes) => {
              ...
              for (const process of processes) {
                if (process.name === 'meilisearch') {
                  this.logger.info(
                    'Connecting to existing Meilisearch service instance...',
                  );
                  resolve(process);
                  return;
                }
              ...
              reject(
                new Error(
                  'Could not find meilisearch process started from main thread.',
                ),
              );
            });
  ...
  async indexMovie(programs: (Movie & HasMediaSourceAndLibraryId)[]) {
    ...
    await this.client()
      .index<ProgramSearchDocument<typeof ProgramType.Movie>>(
        ProgramsIndex.name,
      )
  ...
  async indexShow(show: Show & HasMediaSourceAndLibraryId) {
    ...
    const document: ProgramSearchDocument<typeof ProgramGroupingType.Show> = {
      id: show.uuid,
    ...
    await this.client()
      .index<
        ProgramSearchDocument<typeof ProgramGroupingType.Show>
      >(ProgramsIndex.name)
      .addDocuments([document]);
  }

  async indexSeason(
    season: SeasonWithShow<Season & HasMediaSourceAndLibraryId>,
  ) {
    ...
    const document: ProgramSearchDocument<typeof ProgramGroupingType.Season> = {
      id: season.uuid,
      originalReleaseDate: null,
    ...
    await this.client()
      .index<ProgramSearchDocument<typeof season.type>>(ProgramsIndex.name)
      .addDocuments([document]);
  }

  async indexEpisodes(
    programs: EpisodeWithAncestors2<
      MediaSourceEpisode & HasMediaSourceAndLibraryId
    >[],
  ) {
    ...
    await this.client()
      .index<ProgramSearchDocument<'episode'>>(ProgramsIndex.name)
  ...
  async indexMusicArtist(artist: MusicArtist & HasMediaSourceAndLibraryId) {
    ...
    const document: ProgramSearchDocument<typeof ProgramGroupingType.Artist> = {
      id: artist.uuid,
      originalReleaseDate: null,
      originalReleaseYear: artist.year,
    ...
    await this.client()
      .index<
        ProgramSearchDocument<typeof ProgramGroupingType.Artist>
      >(ProgramsIndex.name)
      .addDocuments([document]);
  }

  async indexMusicAlbum(
    album: AlbumWithArtist<MusicAlbum & HasMediaSourceAndLibraryId>,
  ) {
    ...
    const document: ProgramSearchDocument<typeof ProgramGroupingType.Album> = {
      id: album.uuid,
      originalReleaseDate: null,
      ...
      parent: {
        ...
        year: album.artist.year ?? undefined,
      } satisfies ProgramGroupingDenormDocument<
        typeof ProgramGroupingType.Artist
      >,
    };
    ...
    await this.client()
      .index<ProgramSearchDocument<typeof album.type>>(ProgramsIndex.name)
      .addDocuments([document]);
  }

  async indexMusicTracks(
    tracks: MusicTrackWithAncestors<
      MediaSourceMusicTrack & HasMediaSourceAndLibraryId
    >[],
  ) {
    ...
    const episodeDocuments = tracks.map((program) => {
      ...
      document.grandparent = {
        ...
        year: program.album.artist.year ?? undefined,
      };
      return document;
    });

    await this.client()
      .index<ProgramSearchDocument<'track'>>(ProgramsIndex.name)
      .addDocumentsInBatches(episodeDocuments, 100);
  }
  ...
  private convertProgramToSearchDocument<
    ProgramT extends (Movie | Episode | MusicTrack) &
      HasMediaSourceAndLibraryId,
  >(program: ProgramT): ProgramSearchDocument<NoInfer<ProgramT['type']>> {
    ...
    } satisfies ProgramSearchDocument<typeof program.type>;
  }
