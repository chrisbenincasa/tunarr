...
import { head, isNil, uniq, uniqBy, uniqWith } from 'lodash-es';
import { IAudioMetadata, parseWebStream } from 'music-metadata';
...
export class LocalMusicScanner extends FileSystemScanner {
  ...
  private async scanArtistFolder(
    ...
    const artistResult = await this.loadArtistMetadataFromNfo(fullPath);
    if (artistResult.isFailure()) {
      this.logger.error(artistResult.error);
      return;
    }
  ...
  private async scanAlbum(
    ...
    const metadata = await this.loadAlbumMetadataFromNfo(albumDir);
    if (metadata.isFailure()) {
      return metadata.recast();
    }
    ...
    const trackResults = (
      await Result.attemptAsync(() =>
        this.scanTracks(context, artist, album, albumDir, albumFolderId),
      )
  ...
  private async scanTracks(
    ...
    for (const epFile of allFiles) {
      ...
      const epResult = await Result.attemptAsync(() =>
        this.scanTrack(context, artist, album, epFile, albumFolderId),
      );
  ...
  private async scanTrack(
    ...
    const parsedTrackMetadata = await parseWebStream(
      Readable.toWeb(createReadStream(trackPath)),
      undefined,
      {
        skipCovers: true,
      },
    ...
    // const trackDao = this.programDaoMinter.mintMusicTrack(
    //   context.mediaSource,
    //   context.library,
    //   track,
    //   folderId,
    // );
    // trackDao.program.albumUuid = album.uuid;
    // trackDao.program.artistUuid = artist.uuid;

    // const upsertResult = await this.programDB.upsertPrograms(trackDao);
    // this.logger.debug(
    //   'Upserted track %s (ID = %s)',
    //   upsertResult.title,
    //   upsertResult.uuid,
    ...
    // trackDao.program.uuid = upsertResult.uuid;
    // track.uuid = upsertResult.uuid;

    // await this.searchService.indexMusicTracks([track]);
