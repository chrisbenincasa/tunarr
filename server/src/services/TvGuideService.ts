...
export class TVGuideService {
  ...
  private async buildChannelGuide(
    ...
    return this.locks.getOrCreateLock(channelId).then((lock) =>
      lock.runExclusive(async () => {
        this.logger.info(
          'Building guide info for channel %s (start = %s, duration = %d hours)',
          channelId,
          dayjs(startTime).format(),
          guideDuration.asHours(),
        );
  ...
  private async buildChannelGuideWithRetries(
    ...
    const start = this.currentUpdateTime[channelId]!;
    const end = this.currentEndTime[channelId]!;
    const thisGuideLength = end - start;
    const dur = dayjs.duration(thisGuideLength).humanize();
    this.logger.debug(
      'Building Channel %s for %s (start = %s, end = %s)',
      channelId,
      dur,
      dayjs(start).format(),
      dayjs(end).format(),
