...
import type { RouterPluginCallback, ServerType } from '@/types/serverType.js';
...
@injectable()
export class FfmpegSettingsController extends Controller {
  ...
  protected async configure(fastify: ServerType): Promise<void> {
    ...
    fastify.post(
      ...
      async (req, res) => {
        const copyResult =
          await req.serverCtx.transcodeConfigDB.duplicateConfig(req.params.id);

        if (copyResult.isFailure()) {
          this.logger.error(copyResult.error);
