...
export class ScheduledTask<TaskInputSchemaT extends z.ZodType, OutputT> {
  ...
  async runNow(
    ...
    request?: z.infer<TaskInputSchemaT>,
  ): Promise<OutputT> {
    ...
    if (background) {
      ...
      const result = await this.runJobInternal(request);
      rescheduleCb();
  ...
  protected async runJobInternal(
    request?: z.infer<TaskInputSchemaT>,
  ): Promise<Result<OutputT>> {
    this.running = true;
