...
export async function convertCustomShow(
  ...
  logger.debug(`Migrating ${prettyType}: ${fullPath}`);
...
export async function migrateCustomShows(
  ...
  await mapAsyncSeq(newCustomShows, async (customShow) => {
    ...
    if (type === 'custom-shows') {
      ...
      const maxOrder = maxBy(hasOrder, (c) => c.customOrder)?.customOrder ?? 0;
      ...
      const entities = ld.map(programs, (program, index) =>
        em.create(FillerListContent, {
          ...
          fillerList: em.getReference(FillerShow, entity.uuid as FillerShowId),
        }),
      );
