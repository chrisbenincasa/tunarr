...
} from '../migrations/DirectMigrationProvider.ts';
...
export async function syncMigrationTablesIfNecessary() {
  const tables = await directDbAccess().introspection.getTables({
  ...
  if (!newMigrationTableExists && legacyMigrationTableExists) {
    ...
    const result = await attempt(async () => {
      const previouslyRunLegacyMigrations = await directDbAccess()
      ...
      if (lastRunMigration) {
        ...
        await (directDbAccess() as Kysely<any>)
