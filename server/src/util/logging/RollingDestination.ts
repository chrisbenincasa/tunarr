...
import type { TaskId } from '../../tasks/Task.ts';
...
export class RollingLogDestination {
  public scheduledTask: Maybe<ScheduledTask>;
  private initialized = false;
  private destination!: sonicboom.SonicBoom;
  ...
  initDestination() {
    ...
    if (this.opts.rotateSchedule) {
      const schedule = match(this.opts.rotateSchedule)
        .with({ type: 'cron' }, (schedule) => schedule.cron)
        .with({ type: 'every' }, (schedule) =>
          scheduleRuleToCronString(schedule),
        )
        .exhaustive();
    ...
    this.destination = new sonicboom.SonicBoom({
      ...(this.opts.destinationOpts ?? {}),
      dest: this.opts.fileName,
    });

    this.destination.once('close', () => {
      this.isClosing = true;
  ...
  roll() {
    if (!this.destination || this.isClosing) {
      return;
    }
