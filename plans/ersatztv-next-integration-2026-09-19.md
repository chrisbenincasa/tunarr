...
A VAAPI channel therefore keeps working under `etv_next` and simply stops being a VAAPI channel, with no warning in the UI. The worker does emit one `log::error!` line saying it needs both fields, so this is not wholly silent on its stderr — but nothing reaches the user, and the stream plays either way.

**Filed upstream as [ErsatzTV/next#246](https://github.com/ErsatzTV/next/issues/246)** with a reproduction that needs no media files. The driver is already optional one layer down — `VaapiCapabilities::probe` takes `Option<&str>` and leaves `LIBVA_DRIVER_NAME` alone when it is `None` — and `Vaapi::driver` is read in exactly one place, so the fix upstream is contained.

**Worked around in Tunarr** rather than waiting, in `EtvNextVaapi.ts`. An unset device becomes `/dev/dri/renderD128` on Linux, matching `FfmpegStreamFactory.getVaapiDevice`, and a `system` driver is resolved from the render node's PCI vendor id. The safety argument is that a wrong guess costs nothing: the worker's own probe then fails and it falls back to software, which is exactly what sending no driver already does. When neither can be determined the mapper notes it, so the compatibility notice says the channel will transcode in software instead of staying quiet.
...
| G5  | Infer a default VAAPI device and driver rather than silently dropping hardware acceleration when `vaapi_device` and `vaapi_driver` are not both set. **Filed as [next#246](https://github.com/ErsatzTV/next/issues/246); confirmed empirically 21 Sep 2026** (§11.A) — hardware decode, scaling and encode are all lost, and Tunarr's default `vaapiDriver` of `system` triggers it. |
...
| G5  | **VAAPI silently falls back to software** unless both `vaapi_device` and `vaapi_driver` are set. **Filed as [next#246](https://github.com/ErsatzTV/next/issues/246); confirmed against `ed95077` 21 Sep 2026** (§11.A): the worker emits `libx264` and software `scale` with the driver absent, `h264_vaapi` and `scale_vaapi` with it set. Tunarr owns half of this — the mapper omits `vaapi_driver` for its own `system` default and suppresses the warning, so stock hardware channels quietly stop being hardware channels. Worked around in `EtvNextVaapi.ts` by defaulting the device and inferring the driver from the render node. | `EtvNextChannelConfigMapper.ts:225-232, 279-281`; `TranscodeConfig.ts:148`                                    | `config.rs` VAAPI validation                              | Medium            |
