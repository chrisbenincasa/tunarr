...
> needs its own deep dive before implementation — the survey is accurate about _where_ the
> friction is and unreliable about _how much_ of it there is. See "Fact-check" below.
...
- **#13 — the "argument-swap hazard" does not exist.** Compile-tested: all six cross-assignments of
  ...
  is the _runtime_ context built inside `build()`, not the construction seam the card wants unified.
...
- **#11 — one duplicated block, not a duplicated loop.** The ~14-line midroll block and a TODO
  ...
  is _not_ duplicated inline — both already call the same shared helpers, and RandomSlots factored
...
- **#2 — real gap, milder consequence.** Validation genuinely runs only on the two preview
  ...
  missed: the preview _computes_ `sanitizedSlots` and then discards them — the response returns the
